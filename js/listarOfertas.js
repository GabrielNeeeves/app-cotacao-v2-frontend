class OffersManager {
            ...
            createOfferCard(offer) {
                ...
                card.innerHTML = `
                    ...
                    onclick="offersManager.openUpdateModal(${offer.id}, ${offer.material.id}, '${offer.material.nome}', ${offer.preco}, ${offer.prazoEntrega}, ${offer.quantidadeMinima}, '${(offer.observacoes || '').replace(/'/g, "\\'")}')">
                    Editar
                    </button>
            ...
            openUpdateModal(offerId, materialId, materialNome, preco, prazoEntrega, quantidadeMinima, observacoes) {
                if (!document.getElementById('updateModal')) {
                    this.createUpdateModal();
                }

                document.getElementById('updateOfferId').value = offerId;
                document.getElementById('updateMaterialId').value = materialId; // <-- novo campo hidden
            ...
            async updateOffer() {
            try {
                const funcionarioId = 13; // pode vir do login futuramente
                const offerId = document.getElementById('updateOfferId').value;
                const materialId = document.getElementById('updateMaterialId').value; // <-- pegar aqui
                ...
                const response = await fetch(`${this.apiBaseUrl}/ofertas/${offerId}`, {
                    ...
                    body: JSON.stringify({
                        funcionarioId,
                        materialId: parseInt(materialId), // <-- incluído no JSON
                        preco,
                        prazoEntrega,
                        quantidadeMinima,
                        observacoes

const RoleAuth = {
    ...
    class OfferListsManager {
        ...
        createOfferListCard(offerList, inventory = []) {
            ...
            const firstOffer = offerList.ofertas[0];
        ...
        openDetailsModal(listId) {
            ...
            const firstOffer = list.ofertas[0];
            let companyInfo = '';
            if (firstOffer && firstOffer.funcionario) {
                if (firstOffer.funcionario.empresa) {
                    companyInfo = ` - ${firstOffer.funcionario.empresa.nome}`;
                } else if (firstOffer.funcionario.escola) {
                    companyInfo = ` - ${firstOffer.funcionario.escola.nome}`;
                }
            }
            
            modalTitle.textContent = `Lista de Ofertas #${list.id}${companyInfo}`;
